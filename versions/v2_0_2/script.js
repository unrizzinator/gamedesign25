...
class Tween { // Was assisted on this with ChatGPT. Tweening is something new to me.

    static tweens = [];

    constructor({ from, to, duration = 1000, easing = Tween.linear, onUpdate, onComplete, repeat = 0, backwards = false }) {
        this.from = from;
        this.to = to;
    ...
    tick(deltaTime) {
        ...
        if (t >= 1) {
            ...
            if (this.timesCompleted <= this.repeat) {
                this.elapsed = 0;
                if (this.backwards) {
                    let _tmp = this.from;
                    this.from = this.to;
...
function movePlayerWithCollision(player, obstacles) {
    const steps = Math.ceil(Math.max(Math.abs(player.velocity.x), Math.abs(player.velocity.y)) * COLLISION_RESOLUTION);
    const stepX = player.velocity.x / steps;
    const stepY = player.velocity.y / steps;
...
function updatePhysics(deltaTime) {
    if (player.position.y + player.velocity.y > 5000) reset();
    ...
    movePlayerWithCollision(player, [...Platform.instances, ...Checkpoint.instances]);
...
function draw() {
    ...
    ctx.fillStyle = "#222";
    ctx.fillRect(cW - 43, cH/2 - 203, 26, 406);
    ctx.fillStyle = "#08f";
    ctx.fillRect(cW - 40, cH/2 + 200, 20, -(player.stamina.value/player.stamina.max)*400);
